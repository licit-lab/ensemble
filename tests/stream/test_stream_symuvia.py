...
def no_trajectory_dct(no_trajectory_xml):
    ...
    return XMLTrajectory(no_trajectory_xml)


@pytest.fixture
def one_vehicle_dct(one_vehicle_xml):
    """Dictionary expected answer"""
    return XMLTrajectory(one_vehicle_xml).todict


@pytest.fixture
def two_vehicle_dct(two_vehicle_xml):
    """Dictionary expected answer"""
    return XMLTrajectory(two_vehicle_xml).todict
...
def test_parse_1_vehicle_data(
    ...
    symuviarequest.query = one_vehicle_xml
    veh_data = symuviarequest.get_vehicle_data()
    assert veh_data == tuple((one_trajectory_vehicle_data,))
...
def test_parse_3_get_vehicle_properties(
    ...
    assert set(b0.keys()) == set(one_trajectory_vehicle_data.keys())
    assert set(b1.keys()) == set(one_trajectory_vehicle_data.keys())
