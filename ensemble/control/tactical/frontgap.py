...
from ensemble.component.vehicles.platoonvehicle import PlatoonVehicle
...
class FrontGapCoord(object):
    ...
    def confirm_platoon(self, platoonvehicle):
        if (
            (self.state == Join())
            and (platoonvehicle.relative_speed() < max_relative_speed)
    ...
    def rejoin_platoon(self, platoonvehicle):
        if (
            (self.state == Split())
            and (platoonvehicle.split_request() == False)
            and (platoonvehicle.distance_to_leader < standalone_gap)
