...
class PlatoonVehicle(Vehicle):
    ...
    def joinable(self):
        if (self.leader_platoon_position<self.maximum_platoon_length) and (self.leader_platoon_position + self.platoon_length <= self.maximum_platoon_length) and (
                self.gap_distance_to_leader() < self.max_connection_distance
        ) and self.leader_PCM_capable==True and (self.intruder==False) and (self.leader_split_request==False): # add condition for vehicle split
            return True
        else:
            return False
    ...
    def confirm_platoon(self):
        if (
                abs(self.ego_distance_gap_to_leader - self.desired_gap )< max_gap_distance_error
        ):
            return True
        else:
            return False
    ...
    def rejoin_platoon(self):
        if (
                  (self.intruder == False) #and (self.ego_distance_gap_to_leader <= self.standalone_gap)
    ...
    def leave_platoon(self):
        if (self.ego_distance_gap_to_leader == self.standalone_gap):# distance to cutin vehicle
            return True
        else:
            return False
            # self.state = self.state.switchto("standalone")
